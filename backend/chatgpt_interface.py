...
class Poster:
    def __init__(self, api_key, opinion_axes, max_history=8, dummy_mode=False):
        ...
        if self.dummy_mode:
            print("Poster initialized in DUMMY MODE.")
    ...
    def analyze_post(self, post, max_retries=5):
        if self.dummy_mode:
            print(f"[DUMMY] Analyzing post: '{post[:30]}...'")
            # Return a somewhat consistent but slightly varied opinion based on post length or a hash
            # This makes dummy mode a bit more responsive than pure random for testing.
            val1 = (len(post) % 10) / 10.0
            val2 = (hash(post) % 10) / 10.0
            dummy_vector = [round(val1, 2), round(val2, 2)]
            # Ensure it matches the number of opinion axes
            while len(dummy_vector) < len(self.opinion_axes):
                dummy_vector.append(round(random.uniform(0.3, 0.7), 2))
            print(f"[DUMMY] Opinion vector: {dummy_vector[:len(self.opinion_axes)]}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    # print(f"OpenAI API Error in analyze_post: {e}") # Already printed in main_app
                    raise Exception(f"Failed after {max_retries} attempts: {str(e)}")
                continue
        # print(f"OpenAI API Error: Failed to get valid opinion vector after {max_retries} attempts for post: {post}")
    ...
    def generate_post(self, name, opinion_vector, max_retries=5, is_agent=False):
        # Choose a random topic to post about, as in original logic
        ...
        if self.dummy_mode:
            print(f"[DUMMY] Generating post for {name} on '{axis['name']}' (Opinion: {opinion_on_topic:.2f})")
            ...
            # Add to chat history for context in (future non-dummy) generations
        ...
        if self.chat_history:
            ...
            for entry in self.chat_history[-self.max_history:]: # Use only recent history for prompt length
        ...
        for attempt in range(max_retries):
            try:
                completion = self.client.chat.completions.create(
                    model="gpt-4", # Using gpt-4 as per original for generation
                    messages=[
                        {"role": "system", "content": system_prompt},
                ...
                if attempt == max_retries - 1:
                    # print(f"OpenAI API Error in generate_post: {e}") # Already printed in main_app
                    raise Exception(f"Failed to generate valid post after {max_retries} attempts: {str(e)}")
                continue
        # print(f"OpenAI API Error: Failed to generate post after {max_retries} attempts for {name}")
    ...
    def add_external_post_to_history(self, author_name, post_content):
        """Allows external code (like the main app) to add posts to the history for context."""
        self.chat_history.append({"author": author_name, "post": post_content})
        if len(self.chat_history) > self.max_history:
            self.chat_history.pop(0)
        print(f"[Poster History] Added post from {author_name}: '{post_content[:30]}...'") 